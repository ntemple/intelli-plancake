...
class Nextgen_Core_Transformations_AddConstraint extends Nextgen_Core_Transformation
{
    /*
    ...
    {
      $this->mandatoryParams = array('tableName', 'constraintName');
    ...
    {
        $constraintContent = self::$db2->getTable($this->tableName)->getConstraint($this->constraintName)->getValue();

        return "ALTER TABLE `{$this->tableName}` ADD CONSTRAINT `{$this->constraintName}` ($constraintContent);";
    }
}