...
class Nextgen_Core_Transformations_AddKey extends Nextgen_Core_Transformation
{
    /*
    ...
    {
      $this->mandatoryParams = array('tableName', 'keyName');
    ...
    {
        $keyContent = self::$db2->getTable($this->tableName)->getKey($this->keyName)->getValue();
        return "ALTER TABLE `{$this->tableName}` ADD KEY `{$this->keyName}` ($keyContent);";
    }
}