...
class Nextgen_Core_Transformations_AddIndex extends Nextgen_Core_Transformation
{
    /*
    ...
    {
        $indexContent = self::$db2->getTable($this->tableName)->getIndex($this->indexName)->getValue();
        return "ALTER TABLE `{$this->tableName}` ADD INDEX `{$this->indexName}` ($indexContent);";
    }
}